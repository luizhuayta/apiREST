# Informe Técnico de Pruebas No Funcionales
...
## 2. Fase 1 — Despliegue del Entorno
...
## 3. Fase 2 — Pruebas de Rendimiento

### 3.1 Línea base (usuario único)
...
```
Usuarios    Latencia típica
────────    ───────────────
    1       ~500 ms  ✓ (dentro de 400–600 ms)
   50       ~4,9–6,1 s  ✗ (supera 2 s)
```

Aunque no hubo errores HTTP al 50 % de concurrencia, la latencia deja de ser aceptable. Node.js es **single-thread**: el bucle `while` síncrono en `heavy-process` bloquea el event loop y encola las peticiones entrantes, multiplicando el tiempo de espera de cada cliente.
...
**Condición de parada:** `abortOnFail` cuando la tasa de error supera el 5 %.
...
**Breakpoint identificado:** La API deja de responder de forma fiable con aproximadamente **35–40 usuarios concurrentes** bajo este patrón de carga, muy por debajo de los 200–300 VUs planificados. La latencia efectiva supera los **2 000 ms** y evoluciona hacia **timeouts completos** porque el hilo único no puede atender la cola de peticiones bloqueadas por el bucle síncrono.

**Causa raíz:** El endpoint ejecuta trabajo CPU-bound de forma síncrona:
...
## 4. Fase 3 — Pruebas de Seguridad (OWASP Top 10)
...
### 4.1 Vulnerabilidad 1: Inyección SQL (SQLi)
...
### 4.2 Vulnerabilidad 2: Cross-Site Scripting (XSS)
...
## 5. Fase 4 — Recomendaciones Técnicas

### 5.1 Soluciones de Infraestructura (rendimiento)
...
### 5.2 Soluciones de Código (seguridad)

| Recomendación | Aplica a | Implementación |
|---------------|----------|----------------|
| **Consultas parametrizadas (Prepared Statements)** | SQLi | Usar placeholders en lugar de concatenar: `db.get('SELECT * FROM users WHERE username = ? AND password = ?', [username, password])` |
| **Sanitización de inputs** | SQLi, XSS | Validar y escapar entradas con librerías como `validator`, `DOMPurify` (lado cliente) o `he` (escape HTML en servidor). |
| **Hashing de contraseñas** | SQLi (impacto) | Almacenar contraseñas con `bcrypt`/`argon2`; aunque exista SQLi, no se expondría la contraseña en texto plano. |
| **Cabeceras de seguridad** | XSS | Activar `helmet` en Express: `Content-Security-Policy`, `X-XSS-Protection: 1; mode=block`, `X-Content-Type-Options: nosniff`. |
| **Codificación de salida** | XSS | Escapar datos al serializar para HTML/JSON cuando el consumidor pueda renderizarlos en el DOM. |

### 5.3 Solución de código para el cuello de botella de rendimiento

Reemplazar el bucle síncrono por procesamiento asíncrono o delegado:

```javascript
// Opción A: setImmediate / worker threads para no bloquear el event loop
...
// Opción B: simular carga sin bloquear (solo para desarrollo)
